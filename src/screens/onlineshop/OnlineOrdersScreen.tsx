...
import DatePickerSheet, {
  DatePickerSheetRef,
} from "../../components/common/DatePickerSheet";
...
const OrderRow = React.memo(function OrderRow({
  ...
  s: any;
  currency: string;
  onPress: (order: OnlineOrder) => void;
}) {
    ...
    >
      ...
      >
        <View style={s.cardTop}>
          <View style={{ flex: 1 }}>
            <Text style={[s.customerName, { color: colors.text }]}>
              {order.customerName}
            </Text>
            {order.customerPhone ? (
              <Text style={[s.customerPhone, { color: colors.textMuted }]}>
                {order.customerPhone}
              </Text>
            ) : null}
          ...
          <View style={{ alignItems: "flex-end", gap: 6 }}>
            <Text style={[s.total, { color: colors.text }]}>
...
export default function OnlineOrdersScreen({ navigation, route }: any) {
  ...
  const activeFilterCount =
    (activeTab !== "all" ? 1 : 0) + (periodFilter !== "today" ? 1 : 0);
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <LiquidHeaderIconButton
            ...
            badge={
              activeFilterCount > 0 ? (
                <View
                  style={[s.filterBadge, { backgroundColor: colors.primary }]}
                >
                  <Text style={[s.filterBadgeText, { color: "#fff" }]}>
                    {activeFilterCount}
                  </Text>
                </View>
              ) : undefined
  ...
  const renderOrder = useCallback(
    ({ item, index }: { item: OnlineOrder; index: number }) => (
      <OrderRow
        order={item}
        index={index}
        colors={colors}
  ...
  return (
    <>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoadingConfig || isLoadingOrders ? (
            <OnlineOrdersSkeleton />
          ) : (
            <EmptyState
              icon="bag-outline"
              ...
              subtitle={
                isLoadingOrders
                  ...
                  : searchQuery || activeFilterCount > 0
                    ...
                    : "Orders from your online customers will appear here."
              }
            />
          )
        }
      ...
      <LiquidBottomSheet ref={filterSheetRef}>
        ...
        <Text style={[s.fsSectionLabel, { color: colors.textMuted }]}>
          PERIOD
        </Text>
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  s.fsChip,
                  {
                    backgroundColor: on ? colors.primary : colors.surfaceHigh,
        ...
        {periodFilter === "custom" && (
          ...
          >
            ...
            >
              <View style={{ flex: 1 }}>
                <Text style={[s.fsDateLabel, { color: colors.textMuted }]}>
                  DATE RANGE
                </Text>
                <Text
                  style={[
                    ...
                    {
                      color:
                        customFrom || customTo ? colors.text : colors.textMuted,
                    },
                  ]}
                >
                  {customFrom || customTo
              ...
              <Ionicons
                ...
                size={18}
                color={
                  customFrom && customTo ? colors.success : colors.textMuted
                }
              />
            </TouchableOpacity>
            ...
            {(customFrom || customTo) && (
              ...
              >
                <Ionicons
                  ...
                  color={colors.danger}
                />
                <Text style={[s.fsClearDatesText, { color: colors.danger }]}>
                  Clear dates
                </Text>
              </TouchableOpacity>
            )}
